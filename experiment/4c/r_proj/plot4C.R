...
nt_as_c_mc_reli <- ggplot() +
  #geom_line(data=nt_ch_c_m_data, aes(TIME..ms./1000,(OXIM+ECG+TEMP+ABP)*25, color = "available sensors"), size=0.5) +
  ...
  #geom_line(data=nt_ch_c_m_data, aes(TIME..ms./1000, RELIABILITY*100, color="untamed"), size=1) +
  stat_smooth(data=nt_ch_c_m_data, aes(TIME..ms./1000, RELIABILITY*100, color="untamed"), method = lm, formula = y ~ poly(x, 25), se = FALSE) +
  #geom_line(data=t_ch_c_m_data, aes(TIME..ms./1000, RELIABILITY*100, color="tamed"), size=1) +
  stat_smooth(data=t_ch_c_m_data, aes(TIME..ms./1000, RELIABILITY*100, color="tamed"), method = lm, formula = y ~ poly(x, 25), se = FALSE) +
...
nt_as_c_mc_cost <- ggplot() +
  #geom_line(data=nt_ch_c_m_data, aes(TIME..ms./1000,(OXIM+ECG+TEMP+ABP)*0.175, color="available sensors"), size=0.5) +
  geom_hline(yintercept = cmin, color = 'black', linetype=2, alpha=0.3) +
  geom_hline(yintercept = c, aes(color="set point")) +
  geom_hline(yintercept = cmax, color = 'black', linetype=2, alpha=0.3) +
  #geom_line(data=nt_ch_c_m_data, aes(TIME..ms./1000, COST, color="untamed"), size=1) +
  stat_smooth(data=nt_ch_c_m_data, aes(TIME..ms./1000, COST, color="untamed"), method = lm, formula = y ~ poly(x, 25), se = FALSE) +
  #geom_line(data=t_ch_c_m_data, aes(TIME..ms./1000, COST, color="tamed"), size=1) +
  stat_smooth(data=t_ch_c_m_data, aes(TIME..ms./1000, COST, color="tamed"), method = lm, formula = y ~ poly(x, 25), se = FALSE) +
