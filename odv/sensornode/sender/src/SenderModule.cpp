...
SenderModule::SenderModule(const int32_t &argc, char **argv) :
    TimeTriggeredConferenceClientModule(argc, argv, "SenderModule"),
    buffer(),
    active(true),
    params({{}}),
    ip("localhost"),
...
void SenderModule::setUp() {
    addDataStoreFor(876, buffer);
    ...
    ip = getKeyValueConfiguration().getValue<std::string>("SenderModule.ip");
    ...
    for (string str : sensorsTypes) {
        ...
        lowPercentage  = (op.split(getKeyValueConfiguration().getValue<string>("SenderModule.lowPercentage"), ','));
        midPercentage  = (op.split(getKeyValueConfiguration().getValue<string>("SenderModule.midPercentage"), ','));
        highPercentage = (op.split(getKeyValueConfiguration().getValue<string>("SenderModule.highPercentage"), ','));
...
void SenderModule::tearDown(){    
    sender.disconnect();
}

odcore::data::dmcp::ModuleExitCodeMessage::ModuleExitCode SenderModule::body(){
    string type;
    double data;    
    bool isOK = false;
    TimeData time_data;
    ...
    while (getModuleStateAndWaitForRemainingTimeInTimeslice() == odcore::data::dmcp::ModuleStateMessage::RUNNING) { 
        ...
        while (!buffer.isEmpty()) {    
            ...
            if (container.getDataType() == 902) {

                active = container.getData<SenderModuleControlCommand>().getActive();
                params = container.getData<SenderModuleControlCommand>().getParams();

            } else if (container.getDataType() == 876) {
                ...
                /* Do we still need this ? Does the local controller die ?
